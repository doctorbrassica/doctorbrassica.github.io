...
class MoorhenSynth {
    constructor() {
        ...
        // Pre-generated noise buffer (created during init)
    ...
    async init() {
        ...
        // Pre-generate shared noise buffer for efficiency
    ...
    // Bansuri (flute) voice - uses pre-generated noise buffer
    createBansuriVoice(freq, time) {
        ...
        // Breathy noise - use pre-generated buffer
    ...
    // Play note by instrument with bounds checking
    playNote(instrument, noteIndex) {
        if (!this.isInitialized) return;
        
        // Bounds checking for noteIndex
        const scale = this.scales[this.currentScale];
        const safeIndex = Math.abs(noteIndex) % scale.length;
    ...
    // Wind sound - uses pre-generated noise buffer
    startWind() {
        if (this.windNode) return;
        
        // Use longer buffer for looping ambient sounds
    ...
    startWater() {
        ...
        // Create dedicated buffer for water (longer for variety)
        const bufferSize = this.audioContext.sampleRate * 10;
        const buffer = this.audioContext.createBuffer(1, bufferSize, this.audioContext.sampleRate);
        const data = buffer.getChannelData(0);
        for (let i = 0; i < bufferSize; i++) {
    ...
    // Sequencer with accurate timing using AudioContext
    startSequencer() {
        if (this.sequencer.isPlaying) return;
        
        this.sequencer.isPlaying = true;
        this.sequencer.currentStep = 0;
        this.sequencer.nextStepTime = this.audioContext.currentTime;
        
        // Use requestAnimationFrame-based scheduler for better timing
    ...
    scheduleSequencer() {
        ...
        while (this.sequencer.nextStepTime < this.audioContext.currentTime + lookAhead) {
            this.scheduleStep(this.sequencer.nextStepTime);
            this.advanceStep();
        }
        
        this.sequencer.schedulerTimerId = setTimeout(() => {
            this.scheduleSequencer();
        }, scheduleInterval);
    }
    ...
    scheduleStep(time) {
        ...
        instruments.forEach(inst => {
            ...
            if (stepData && stepData.active) {
                ...
                const scale = this.scales[this.currentScale];
                const safeIndex = Math.abs(stepData.noteIndex) % scale.length;
                const freq = scale[safeIndex];
                
                switch (inst) {
                    case 'bowls':
                        this.createBowlVoice(freq, time);
                        break;
                    case 'wind':
                        this.createBansuriVoice(freq, time);
                        break;
                    case 'strings':
                        this.createSitarVoice(freq, time);
                        break;
                    case 'horn':
                        this.createHornVoice(freq, time);
                        break;
                    case 'moorhen':
                        this.createMoorhenVoice(freq, time);
                        break;
                    case 'oystercatcher':
                        this.createOystercatcherVoice(freq, time);
                        break;
                }
            }
        });
    ...
    advanceStep() {
        const stepDuration = 60 / this.sequencer.bpm / 4; // 16th notes in seconds
        this.sequencer.nextStepTime += stepDuration;
    ...
    stopSequencer() {
        ...
        if (this.sequencer.schedulerTimerId) {
            clearTimeout(this.sequencer.schedulerTimerId);
            this.sequencer.schedulerTimerId = null;
        }
    }
    
    setSequencerTempo(bpm) {
        this.sequencer.bpm = bpm;
        // No need to restart - the scheduler will pick up the new tempo automatically
    }
    ...
    cleanup() {
        ...
        if (this.waterNode) {
            try {
                this.waterNode.noise.stop();
                this.waterNode.noise2.stop();
