...
// Get note names from current scale
function getNoteNames() {
    if (synth) {
        return synth.getScaleInfo().noteNames;
    }
    return ['C', 'D#', 'F', 'G', 'A#', 'C+', 'D#+'];
}
...
function updateNoteDropdowns() {
    ...
    document.querySelectorAll('.note-select').forEach(select => {
        ...
        options.forEach((option, i) => {
            if (i > 0) {
                option.textContent = noteNames[i - 1];
            }
        });
...
function updateInstrumentNoteLabels() {
    const noteNames = getNoteNames();
    document.querySelectorAll('.mini-notes').forEach(container => {
        container.querySelectorAll('[data-note]').forEach(el => {
            const noteIndex = parseInt(el.dataset.note);
            const span = el.querySelector('span');
            if (span && noteNames[noteIndex]) {
                span.textContent = noteNames[noteIndex];
            }
        });
    });
}
...
function setupScaleSelector() {
    ...
    scaleSelect.addEventListener('change', () => {
        const scaleInfo = synth.setScale(scaleSelect.value);
        if (scaleInfo) {
            updateNoteDropdowns();
        }
    });
...
function getDefaultPatch() {
    return {
        ...
        tempo: 120,
        randomness: 0,
        drone: { active: false, base: 0, volume: 0.3 },
        ambient: { wind: 0, water: 0 },
        expression: { filter: 2000, reverb: 0.5 }
    };
}
...
function serializePatch() {
    const patch = {
        version: PATCH_VERSION,
        scale: synth ? synth.currentScale : 'pentatonic',
        patterns: {},
        volumes: {
            master: parseFloat(document.getElementById('master-volume').value)
        },
        tempo: sequencer.tempo,
        randomness: sequencer.randomness,
        drone: {
            active: synth ? synth.droneActive : false,
            base: parseInt(document.getElementById('drone-base').value),
            volume: parseFloat(document.getElementById('drone-volume').value)
        },
        ambient: {
            wind: parseFloat(document.getElementById('wind-volume').value),
            water: parseFloat(document.getElementById('water-volume').value)
        },
        expression: {
            filter: synth ? synth.filterNode.frequency.value : 2000,
            reverb: synth ? synth.reverbGain.gain.value : 0.5
        }
    };

    Object.keys(sequencer.patterns).forEach(instrument => {
        patch.patterns[instrument] = [...sequencer.patterns[instrument]];
    });

    if (synth) {
        Object.keys(synth.instrumentVolumes).forEach(instrument => {
            patch.volumes[instrument] = synth.instrumentVolumes[instrument];
        });
    }
...
function deserializePatch(str) {
    str = str.trim();
    if (!str.startsWith(PATCH_PREFIX)) {
        throw new Error('Not a valid Moorhen patch string');
    }

    const jsonStr = str.slice(PATCH_PREFIX.length).trim();

    let patch;
    try {
        patch = JSON.parse(jsonStr);
    } catch (e) {
        throw new Error('Invalid patch data (bad JSON)');
    }

    if (!patch.version || !patch.patterns || !patch.volumes) {
        throw new Error('Invalid patch data (missing fields)');
    }
...
function applyPatch(patch) {
    ...
    // Scale
    if (patch.scale && synth) {
        synth.setScale(patch.scale);
        const scaleSelect = document.getElementById('scale-select');
        if (scaleSelect) {
            scaleSelect.value = patch.scale;
        }
        updateNoteDropdowns();
    }

    // Patterns
    const instruments = ['bowls', 'wind', 'strings', 'horn', 'moorhen', 'oystercatcher', 'stick'];
    instruments.forEach(inst => {
        if (patch.patterns[inst]) {
            sequencer.patterns[inst] = [...patch.patterns[inst]];
        }
    });

    // Update sequencer grid UI
    document.querySelectorAll('.seq-row').forEach(row => {
        const instrument = row.dataset.instrument;
        if (!instrument || !sequencer.patterns[instrument]) return;
        row.querySelectorAll('.seq-step').forEach(step => {
            const stepIdx = parseInt(step.dataset.step);
            const select = step.querySelector('.note-select');
            const noteVal = sequencer.patterns[instrument][stepIdx];
            if (noteVal !== null && noteVal !== undefined) {
                step.classList.add('active');
                if (select) select.value = noteVal;
            } else {
                step.classList.remove('active');
    ...
    // Master volume
    if (patch.volumes && patch.volumes.master !== undefined) {
        const masterSlider = document.getElementById('master-volume');
        masterSlider.value = patch.volumes.master;
        if (synth) synth.setMasterVolume(patch.volumes.master);
    }

    // Instrument volumes
    instruments.forEach(inst => {
        if (patch.volumes && patch.volumes[inst] !== undefined) {
            if (synth) synth.setInstrumentVolume(inst, patch.volumes[inst]);
            const slider = document.querySelector(`.inst-volume-slider[data-instrument="${inst}"]`);
            if (slider) slider.value = patch.volumes[inst];
        }
    });

    // Tempo
    if (patch.tempo !== undefined) {
        sequencer.tempo = patch.tempo;
        const tempoSlider = document.getElementById('seq-tempo');
        const tempoValue = document.getElementById('seq-tempo-value');
        tempoSlider.value = patch.tempo;
        tempoValue.textContent = `${patch.tempo} BPM`;
    }

    // Randomness
    if (patch.randomness !== undefined) {
        sequencer.randomness = patch.randomness;
        const randomSlider = document.getElementById('seq-randomness');
        const randomValue = document.getElementById('seq-randomness-value');
        randomSlider.value = patch.randomness;
        randomValue.textContent = `${Math.round(patch.randomness * 100)}%`;
    }
    ...
    if (patch.drone) {
        ...
        droneBase.value = patch.drone.base;
        const noteIndex = ((patch.drone.base % 12) + 12) % 12;
        const octave = Math.floor((patch.drone.base + 24) / 12);
        droneBaseValue.textContent = chromNotes[noteIndex] + octave;
        if (synth) synth.setDroneBase(patch.drone.base);

        droneVolume.value = patch.drone.volume;
        if (synth) synth.setDroneVolume(patch.drone.volume);

        if (synth) {
            const droneCurrentlyActive = synth.droneActive;
            if (patch.drone.active && !droneCurrentlyActive) {
                droneBtn.click();
            } else if (!patch.drone.active && droneCurrentlyActive) {
                droneBtn.click();
            }
        }
    }
    ...
    if (patch.ambient) {
        ...
        if (patch.ambient.wind !== undefined) {
            windSlider.value = patch.ambient.wind;
            windValue.textContent = `${Math.round(patch.ambient.wind * 100)}%`;
            if (synth) synth.setWindVolume(patch.ambient.wind * 0.3);
        }
        if (patch.ambient.water !== undefined) {
            waterSlider.value = patch.ambient.water;
            waterValue.textContent = `${Math.round(patch.ambient.water * 100)}%`;
            if (synth) synth.setWaterVolume(patch.ambient.water * 0.2);
        }
    }

    // Expression (filter & reverb)
    if (patch.expression) {
        if (synth && patch.expression.filter !== undefined) {
            synth.setFilter(patch.expression.filter);
        }
        if (synth && patch.expression.reverb !== undefined) {
            synth.setReverb(patch.expression.reverb);
        }

        // Update XY pad cursor to reflect loaded expression values
        const pad = document.getElementById('xy-pad');
        const cursor = document.getElementById('xy-cursor');
        const filterValue = document.getElementById('filter-value');
        const reverbValue = document.getElementById('reverb-value');
        if (pad && cursor) {
            const relX = Math.log(patch.expression.filter / 200) / Math.log(40);
            const relY = 1 - patch.expression.reverb;
            cursor.style.left = `${Math.max(0, Math.min(1, relX)) * 100}%`;
            cursor.style.top = `${Math.max(0, Math.min(1, relY)) * 100}%`;
            if (filterValue) filterValue.textContent = `Filter: ${Math.round(patch.expression.filter)} Hz`;
            if (reverbValue) reverbValue.textContent = `Space: ${Math.round(patch.expression.reverb * 100)}%`;
        }
    }
...
function showPatchFeedback(message, isError) {
    const feedback = document.getElementById('patch-feedback');
    feedback.textContent = message;
    feedback.className = 'patch-feedback ' + (isError ? 'error' : 'success');
    feedback.classList.remove('hidden');
    setTimeout(() => feedback.classList.add('hidden'), 3000);
}

function setupPatchModal() {
    const patchBtn = document.getElementById('patch-btn');
    const modal = document.getElementById('patch-modal');
    const closeBtn = document.getElementById('patch-modal-close');
    const backdrop = modal.querySelector('.modal-backdrop');
    const textarea = document.getElementById('patch-textarea');
    const copyBtn = document.getElementById('patch-copy-btn');
    const loadBtn = document.getElementById('patch-load-btn');
    const clearBtn = document.getElementById('patch-clear-btn');

    function openModal() {
        modal.classList.remove('hidden');
        textarea.value = serializePatch();
    }
    ...
    patchBtn.addEventListener('click', openModal);
    closeBtn.addEventListener('click', closeModal);
    backdrop.addEventListener('click', closeModal);
    ...
    copyBtn.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(textarea.value).then(() => {
            showPatchFeedback('Patch copied to clipboard', false);
        }).catch(() => {
            document.execCommand('copy');
            showPatchFeedback('Patch copied', false);
        });
    });

    loadBtn.addEventListener('click', () => {
        try {
            const patch = deserializePatch(textarea.value);
            applyPatch(patch);
            showPatchFeedback('Patch loaded successfully', false);
        } catch (e) {
            showPatchFeedback(e.message, true);
        }
    });

    clearBtn.addEventListener('click', () => {
        const defaults = getDefaultPatch();
        applyPatch(defaults);
        textarea.value = serializePatch();
        showPatchFeedback('Reset to defaults', false);
    });
}
